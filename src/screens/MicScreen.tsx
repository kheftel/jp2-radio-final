...
import { useAudioRecorder, useAudioPlayer, RecordingPresets, requestRecordingPermissionsAsync, AudioPlayer } from 'expo-audio';
...
export default function MicScreen() {
  ...
  const player = useConditionalAudioPlayer(recordingUri);
  ...
  const startRecording = async () => {
    try {
      ...
      if (status !== 'granted') {
        Alert.alert('Permission needed', 'Please allow microphone access to record messages.');
        return;
      }
      ...
      <View style={styles.content}>
        ...
        <View style={styles.buttonContainer}>
          ...
          {recordingState === 'review' && (
            <TouchableOpacity style={styles.reviewButton} onPress={togglePlayback}>
        ...
        {(recordingState === 'recording' || recordingState === 'review') && (
          <View style={styles.timerContainer}>
            ...
            {recordingState === 'recording' && <Text style={styles.maxTime}>2:00 max</Text>}
          </View>
        )}

        {recordingState === 'idle' && <Text style={styles.tapText}>Tap to record</Text>}
        {recordingState === 'recording' && (
...
const styles = StyleSheet.create({
  ...
  title: { color: Colors.white, fontSize: FontSizes.title, fontWeight: '700', marginBottom: Spacing.sm },
  subtitle: { color: Colors.textSecondary, fontSize: FontSizes.md, marginBottom: Spacing.xxxl },
  ...
  maxTime: { color: Colors.textSecondary, fontSize: FontSizes.sm, marginTop: 4 },
  ...
  reviewActions: { width: '100%', paddingHorizontal: Spacing.xl, gap: Spacing.md },
  sendButton: {
    backgroundColor: Colors.primary,
  ...
  sendButtonText: { color: Colors.white, fontSize: FontSizes.lg, fontWeight: '600' },
  rerecordButton: {
    backgroundColor: Colors.surfaceLight,
    borderWidth: 1,
    borderColor: Colors.border,
  ...
  rerecordButtonText: { color: Colors.white, fontSize: FontSizes.lg, fontWeight: '500' },
  discardText: { color: Colors.textSecondary, fontSize: FontSizes.md, textAlign: 'center', paddingTop: Spacing.md },
  tipsBox: {
